...
export class SendMessage extends Entity {
  constructor(id: Bytes) {
    super();
    this.set("id", Value.fromBytes(id));
  }
  ...
  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save SendMessage entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.BYTES,
        `Entities of type SendMessage must have an ID of type Bytes but the id '${id.displayData()}' is of type ${id.displayKind()}`
      );
      store.set("SendMessage", id.toBytes().toHexString(), this);
    }
  }

  static loadInBlock(id: Bytes): SendMessage | null {
    return changetype<SendMessage | null>(
      store.get_in_block("SendMessage", id.toHexString())
    );
  }

  static load(id: Bytes): SendMessage | null {
    return changetype<SendMessage | null>(
      store.get("SendMessage", id.toHexString())
    );
  }
